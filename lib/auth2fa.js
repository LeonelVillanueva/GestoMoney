...
/** Token opaco de confianza de dispositivo (solo existe en cookie HTTP-only; en BD va el hash). */
export function generateTrustTokenRaw() {
  return crypto.randomBytes(32).toString('base64url')
}

export function hashTrustToken(rawToken) {
  return hashWithPepper(`trust-device:${String(rawToken || '')}`)
}
